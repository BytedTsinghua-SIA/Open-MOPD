...
try:  # vLLM < 0.11 exposed these at the top level of vllm.utils
    from vllm.utils import FlexibleArgumentParser, get_tcp_uri
except ImportError:  # vLLM >= 0.11 split vllm.utils into a package (verified on 0.18.0)
...
def _resolve_max_model_len(config: RolloutConfig) -> int:
    """Honor an explicit rollout context window and derive only a fallback.

    Validation may use a longer prompt/response profile than training.  The
    previous unconditional assignment reduced every async vLLM server to the
    *training* ``prompt_length + response_length`` (for IF OPD, 4096), even
    when the recipe explicitly requested 32768.  That made valid 10K--16K
    formal prompts produce a negative ``max_tokens`` before generation.
    """
    configured = config.max_model_len
    if configured is not None:
        configured = int(configured)
        if configured <= 0:
            raise ValueError(f"max_model_len must be positive, got {configured}")
        return configured

    derived = int(config.prompt_length) + int(config.response_length)
    if derived <= 0:
        raise ValueError(
            "prompt_length + response_length must be positive when max_model_len is unset, "
            f"got {derived}"
        )
    return derived
...
class ExternalZeroMQDistributedExecutor(Executor):
    ...
    def collective_rpc(
        ...
        **kwargs_extra: Any,
        ...
        # vLLM 0.18: EngineCore.step() calls execute_model(..., non_block=True) and then
        # `future.result()`. The base Executor.execute_model forwards non_block to
        # collective_rpc and returns output[0], so for non_block we must return Futures.
        # Our ZMQ RPC is synchronous, so wrap each already-computed result in a completed
        # Future to satisfy the contract (no overlap, but correct).
        if non_block:
            wrapped: list[Future] = []
            for output in outputs:
                fut: Future = Future()
...
class vLLMHttpServerBase:
    ...
    def __init__(
        ...
        self.config.max_model_len = _resolve_max_model_len(self.config)
    ...
    async def launch_server(self, master_address: str = None, master_port: int = None):
        ...
        args = {
            ...
            "seed": ranked_rollout_seed(self.config.get("seed", 0), self.replica_rank),
    ...
    async def generate(
        ...
        # Each request owns its SamplingParams conversion.  In particular,
        # popping max_tokens/logprobs here must not mutate a dict reused by a
        # multi-turn agent loop.
        sampling_params = dict(sampling_params)
        max_available_tokens = self.config.max_model_len - len(prompt_ids)
        requested_max_tokens = sampling_params.pop("max_tokens", None)
        max_tokens = (
            max_available_tokens
            if requested_max_tokens is None
            else min(max(1, int(requested_max_tokens)), max_available_tokens)
        )
        sampling_params["logprobs"] = 0 if sampling_params.pop("logprobs", False) else None
        sampling_params.setdefault("repetition_penalty", self.config.get("repetition_penalty", 1.0))
        if sampling_params.get("stop_token_ids") is not None and not isinstance(
