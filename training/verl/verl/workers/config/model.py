...
class HFModelConfig(BaseConfig):
    ...
    # dtype for model initialization
    dtype: str = "bf16"
    ...
    def __post_init__(self):
        ...
        generation_config_path = self.local_hf_config_path
        if self.local_tokenizer_path is not None and os.path.isfile(
            os.path.join(self.local_tokenizer_path, "generation_config.json")
        ):
            # A separate post-training tokenizer package owns the chat-template
            # turn terminators. Prefer its generation contract over the Base
            # model's pretraining EOS metadata.
            generation_config_path = self.local_tokenizer_path
        self.generation_config = get_generation_config(generation_config_path, trust_remote_code=self.trust_remote_code)
