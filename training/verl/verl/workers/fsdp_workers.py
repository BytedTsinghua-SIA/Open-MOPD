...
def _compute_student_topk_in_teacher_top_p_mask(
    ...
    if student_ids.dim() != 2:
        raise ValueError(f"student_ids must be [N, K], got {tuple(student_ids.shape)}")
    if logits.shape[0] != student_ids.shape[0]:
        raise ValueError(f"logits and student_ids batch dims must match, got {logits.shape[0]} and {student_ids.shape[0]}")
    ...
    full_mask = torch.zeros_like(sorted_mask, dtype=torch.bool)
    full_mask.scatter_(dim=-1, index=sorted_ids, src=sorted_mask)
    return torch.gather(full_mask, dim=-1, index=student_ids).float()
...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    async def rollout_mode(self):
        ...
        if skip_initial_weight_sync and self._rollout_weight_sync_count == 0:
            if "dummy" in self.config.rollout.load_format:
                raise ValueError("skip_initial_weight_sync requires a checkpoint-preloaded rollout.load_format")
            logger.warning(
                "OPENOPD_DIAGNOSTIC skipping the first actor -> rollout weight synchronization; "
    ...
    @register(dispatch_mode=make_nd_compute_dataproto_dispatch_fn(mesh_name="actor"))
    @DistProfiler.annotate(color="blue", role="actor_compute_delta_opd_reward")
    def compute_delta_opd_reward(self, data: DataProto):
        assert self._is_actor
        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.actor_module_fsdp)

        with self.ulysses_sharding_manager:
            output = self.actor.compute_delta_opd_reward(data=data)
        ...
        if self._is_offload_param:
            offload_fsdp_model_to_cpu(self.actor_module_fsdp)
            log_gpu_memory_usage("After offload actor model during compute_delta_opd_reward", logger=logger)

        return output

    @register(dispatch_mode=make_nd_compute_dataproto_dispatch_fn(mesh_name="actor"))
    @DistProfiler.annotate(color="blue", role="actor_compute_exopd_reward")
    def compute_exopd_reward(self, data: DataProto):
        assert self._is_actor
        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.actor_module_fsdp)

        with self.ulysses_sharding_manager:
            output = self.actor.compute_exopd_reward(data=data)
        ...
        if self._is_offload_param:
            offload_fsdp_model_to_cpu(self.actor_module_fsdp)
            log_gpu_memory_usage("After offload actor model during compute_exopd_reward", logger=logger)
    ...
    def load_checkpoint(self, local_path, remote_path=None, del_local_after_load=False):
        ...
        self.checkpoint_manager.load_checkpoint(
            local_path=local_path, remote_path=remote_path, del_local_after_load=del_local_after_load
        )
...
class CriticWorker(Worker, DistProfilerExtension):
    ...
    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def load_checkpoint(self, local_path, remote_path=None, del_local_after_load=True):
        import torch
...
class RewardModelWorker(Worker, DistProfilerExtension):
    ...
    def _build_model(self, config):
        ...
        use_shm = config.model.get("use_shm", False)
        # download the checkpoint from remote
        local_path = copy_to_local(config.model.path, use_shm=use_shm)
    ...
    def _compute_teacher_top_k_log_probs(
        ...
        for start in range(0, n_samples, chunk_size):
            ...
            if strategy in ["only_stu", "union","union-intersection", "top_p_intersec", "top-p-intersec"]:
                ...
                if strategy in ["top_p_intersec", "top-p-intersec"]:
                    overlap_mask_chunk = _compute_student_topk_in_teacher_top_p_mask(
                        logits=logits_chunk,
                        student_ids=student_ids_chunk,
                        top_p=top_p_intersec_p,
                    )
                    overlap_counts_list[-1] = overlap_mask_chunk
    ...
    def compute_rm_score(self, data: DataProto, kl_estimator="k1"):
        ...
        with self.ulysses_sharding_manager:
            ...
            top_p_intersec_p = data.meta_info.get(
                "top_p_intersec_p", self.config.get("top_p_intersec_p", 0.99)
            )
