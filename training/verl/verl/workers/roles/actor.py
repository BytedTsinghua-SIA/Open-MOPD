...
class ActorWorker(Worker, DistProfilerExtension):
    ...
    def save_checkpoint(self, local_path, remote_path=None, global_step=0, max_ckpt_to_keep=None):
        return self.engine.save_checkpoint(local_path, remote_path, global_step, max_ckpt_to_keep)

    @register(dispatch_mode=Dispatch.ONE_TO_ALL)
    def load_checkpoint(self, local_path, remote_path=None, del_local_after_load=False):
        return self.engine.load_checkpoint(local_path, remote_path, del_local_after_load)
