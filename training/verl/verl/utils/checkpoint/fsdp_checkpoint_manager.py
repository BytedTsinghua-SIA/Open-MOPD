...
from verl.utils.local_fs import copy_to_local, local_mkdir_safe
...
class FSDPCheckpointManager(BaseCheckpointManager):
    ...
    def load_checkpoint(self, local_path: str, remote_path: str = None, del_local_after_load=False):
        """
        ...
        Args:
            ...
            remote_path: Unused compatibility argument.
        ...
        with get_fsdp_state_ctx(self.model, StateDictType.SHARDED_STATE_DICT, state_dict_cfg, optim_cfg):
            if self.should_load_model:
                ...
                local_model_path = copy_to_local(checkpoint_model_path)
                model_state_dict = torch.load(local_model_path, weights_only=False)
                self.model.load_state_dict(model_state_dict)
                log_with_rank(f"Loaded model from {checkpoint_model_path}", rank=self.rank, logger=logger)

            if self.should_load_optimizer:
                checkpoint_optim_path = os.path.join(
                    local_path, f"optim_world_size_{self.world_size}_rank_{self.rank}.pt"
                )
                local_optim_path = copy_to_local(checkpoint_optim_path)
                optimizer_state_dict = torch.load(local_optim_path, weights_only=False)
                self.optimizer.load_state_dict(optimizer_state_dict)
                log_with_rank(f"Loaded optimizer from {checkpoint_optim_path}", rank=self.rank, logger=logger)

        if self.should_load_extra:
            checkpoint_extra_state_path = os.path.join(
                local_path, f"extra_state_world_size_{self.world_size}_rank_{self.rank}.pt"
            )
            local_extra_state_path = copy_to_local(checkpoint_extra_state_path)
            extra_state_dict = torch.load(local_extra_state_path, weights_only=False)
            # recover random state
            if "rng" in extra_state_dict:
                # 'rng' may not exist for backward compatibility
                self.load_rng_state(extra_state_dict["rng"])
                log_with_rank(f"Loaded rng from {checkpoint_extra_state_path}", rank=self.rank, logger=logger)

            lr_scheduler_state_dict = extra_state_dict["lr_scheduler"]
            if lr_scheduler_state_dict is not None and self.lr_scheduler is not None:
                self.lr_scheduler.load_state_dict(lr_scheduler_state_dict)
                log_with_rank(f"Loaded lr_scheduler from {checkpoint_extra_state_path}", rank=self.rank, logger=logger)

        # wait for everyone to load checkpoints
        torch.distributed.barrier()

    def save_checkpoint(self, local_path: str, remote_path: str = None, global_step: int = 0, max_ckpt_to_keep=None):
        """
        ...
        Args:
            ...
            remote_path: Unused compatibility argument; checkpoints stay local.
        ...
        if self.rank == 0:
            ...
            if unwrap_model.can_generate():
                processing_class = self.processing_class
                if processing_class is not None and hasattr(processing_class, "tokenizer"):
                    processing_class = processing_class.tokenizer
                generation_config_sources = [getattr(processing_class, "name_or_path", None)]
                generation_config_sources.append(getattr(model_config, "name_or_path", None))
                for source in dict.fromkeys(source for source in generation_config_sources if source):
                    try:
                        # A separate post-training tokenizer package may define
                        # turn-end stop IDs that differ from the Base model EOS.
                        generation_config = GenerationConfig.from_pretrained(source)
                        break
                    except Exception:
                        continue
            ...
            rank0_metadata_paths.extend([hf_config_tokenizer_path, fsdp_config_path])
