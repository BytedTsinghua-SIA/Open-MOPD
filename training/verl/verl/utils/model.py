...
def get_hf_model_path(config):
    ...
    return copy_to_local(src=config.model.path, use_shm=config.model.get("use_shm", False))
...
_architecture_to_auto_class = {
    ...
    # Multimodal decoder models such as Gemma 3 expose a
    # ``*ForConditionalGeneration`` architecture while still implementing the
    # causal language-modeling interface needed by SFT/RL.  Falling back to
    # ``AutoModel`` drops the LM head and produces no token logits.
