...
def get_fsdp_wrap_policy(module, config=None, is_lora=False):
    ...
    configured_transformer_cls_names_to_wrap = _get_attr("transformer_layer_cls_to_wrap", None)
    using_model_defaults = configured_transformer_cls_names_to_wrap is None
    fsdp_transformer_layer_cls_to_wrap = (
        default_transformer_cls_names_to_wrap
        if using_model_defaults
        else configured_transformer_cls_names_to_wrap
    )
    ...
    if min_num_params > 0:
        ...
        for layer_class in fsdp_transformer_layer_cls_to_wrap:
            ...
            if transformer_cls is None:
                # Composite HF model classes can list optional components in
                # `_no_split_modules` even when a particular checkpoint does
                # not instantiate them (for example text-only Gemma 3 has no
                # SigLIP vision tower). Model-owned defaults are therefore
                # filtered to the classes that are actually present. An
                # explicitly configured class name remains strict so typos do
                # not silently change the sharding policy.
                if using_model_defaults:
                    continue
                raise Exception(f"Could not find transformer layer class {layer_class!r} to wrap in the model.")
            else:
