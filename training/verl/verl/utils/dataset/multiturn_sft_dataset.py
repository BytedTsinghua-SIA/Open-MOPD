...
from verl.utils.local_fs import copy_local_path
...
class MultiTurnSFTDataset(Dataset):
    ...
    def _download(self):
        for i, parquet_file in enumerate(self.parquet_files):
            self.parquet_files[i] = copy_local_path(parquet_file, verbose=True)
    ...
    def _apply_chat_template(
        ...
        **kwargs,
        ...
        """Apply the tokenizer template without overriding its native defaults.

        Some model families (notably SmolLM3) distinguish between an omitted
        ``enable_thinking`` argument and ``enable_thinking=None``.  Passing the
        latter disables the template's default thinking mode and can inject an
        empty ``<think>`` block in front of responses that already contain a
        complete thinking block.  Only forward the per-row value when the data
        actually provides one; otherwise preserve an explicit config-level value
        or let the tokenizer template choose its native default.
        """
    ...
    def __getitem__(self, item):
        ...
        try:
            full_tokens = self._apply_chat_template(
                messages,
                tokenize=True,
                add_generation_prompt=False,
                enable_thinking=enable_thinking,
