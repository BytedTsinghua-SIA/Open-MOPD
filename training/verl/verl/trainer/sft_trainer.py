...
def run_sft(config):
    ...
    try:
        ...
        # Print the primary per-rank failure before distributed teardown.  A
        # failed rank can otherwise block in destroy_process_group while its
        # peers only report a secondary NCCL watchdog timeout.
        traceback.print_exc()
        raise
    finally:
