...
class AlgoConfig(BaseConfig):
    ...
    Args:
        ...
        mask_truncated_samples (bool): Whether to leave zero-reward samples with full-length responses at zero
            advantage instead of normalizing them with other completions in the group.
        overlong_filter (bool): DAPO Overlong Filtering. Whether to drop EVERY truncated (full-length) sample
            from the GRPO group statistics and give it zero advantage, regardless of reward, i.e. treat
            truncated completions as if they were never rolled out.
        clip_advantages (bool): Whether to clamp computed advantages to [-1, 1].
