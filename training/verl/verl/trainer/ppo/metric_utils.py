...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    reward_score_component_metrics: dict[str, Any] = {}
    for component_name in ("think_score", "answer_score", "final_score"):
        if component_name not in batch.non_tensor_batch:
            continue
        try:
            component = torch.as_tensor(batch.non_tensor_batch[component_name], dtype=torch.float32)
        except (TypeError, ValueError):
            continue
        if component.numel() == 0:
            continue
        reward_score_component_metrics.update(
            {
                f"score/{component_name}/mean": component.mean().detach().item(),
                f"score/{component_name}/max": component.max().detach().item(),
                f"score/{component_name}/min": component.min().detach().item(),
            }
        )
    ...
    metrics = {
        ...
        **reward_score_component_metrics,
        # reward
