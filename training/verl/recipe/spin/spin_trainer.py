...
class RaySPINTrainer:
    ...
    def _save_checkpoint(self):
        ...
        actor_remote_path = (
            None
            if self.config.trainer.default_remote_dir is None
            else os.path.join(self.config.trainer.default_remote_dir, f"global_step_{self.global_steps}", "actor")
        )
        ...
        if self.use_critic:
            ...
            critic_remote_path = (
                None
                if self.config.trainer.default_remote_dir is None
                else os.path.join(self.config.trainer.default_remote_dir, f"global_step_{self.global_steps}", "critic")
            )
    ...
    def _load_checkpoint(self):
        ...
        # load from remote
        if self.config.trainer.default_remote_dir is not None:
            raise NotImplementedError("load from remote is not implemented yet")
        else:
