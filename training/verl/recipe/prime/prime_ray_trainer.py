...
class RayPRIMETrainer(RayPPOTrainer):
    ...
    def _save_checkpoint(self):
        ...
        if self.use_rm:
            ...
            reward_remote_path = (
                None
                if self.config.trainer.default_remote_dir is None
                else os.path.join(self.config.trainer.default_remote_dir, f"global_step_{self.global_steps}", "reward")
            )
    ...
    def _load_checkpoint(self):
        ...
        if self.config.trainer.default_remote_dir is not None:
            NotImplementedError("load from remote is not implemented yet")
        else:
