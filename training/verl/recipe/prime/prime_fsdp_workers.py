...
class PRIMERewardModelWorker(Worker):
    ...
    def _build_reward_ref_model_optimizer(self, config):
        ...
        local_path = copy_local_path(config.model.path)

        tokenizer_path = copy_local_path(config.model.tokenizer_path)
        ...
        with init_context(), warnings.catch_warnings():
            ...
            ref_module = AutoModelForCausalLM.from_pretrained(
                pretrained_model_name_or_path=copy_local_path(config.model.ref_path),
                torch_dtype=torch_dtype,
                config=reward_model_config,
                attn_implementation="flash_attention_2",
                trust_remote_code=trust_remote_code,
