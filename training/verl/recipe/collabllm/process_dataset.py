...
def maybe_copy_to_remote(local_dir: str, remote_dir: Optional[str]) -> None:
    if not remote_dir:
        return
    try:
        from verl.utils.local_io import copy, makedirs  # type: ignore
    except Exception as e:
        print(f"[WARN] Skipping remote storage copy (verl not available): {e}")
        return
    makedirs(remote_dir)
    copy(src=local_dir, dst=remote_dir)
    print(f"[OK] Copied {local_dir} → {remote_dir}")
...
def main():
    ...
    ap.add_argument("--remote_dir", default=None, help="Optional remote storage destination (requires verl).")
    ap.add_argument(
    ...
    maybe_copy_to_remote(local_dir=out_dir, remote_dir=args.remote_dir)
